...
- **Biome 2** replaces ESLint + Prettier + Stylelint (single Rust binary, faster, simpler).
...
| React | 18 | 19 |
| Lint + format + CSS lint | Next.js default (`next lint`), no formatter | **Biome 2** (single Rust binary replacing ESLint + Prettier + Stylelint) |
...
### Biome over ESLint + Prettier + Stylelint

Originally adopted ESLint 9 flat config + Prettier + Stylelint. Hit a hard ceiling: when bumping to **ESLint 10**, the entire TypeScript-aware plugin chain (`typescript-eslint@8.x`, the latest stable, used by `eslint-config-next@16`) calls `scopeManager.addGlobals()` which ESLint 10 removed from its internal API. `typescript-eslint@9` has not been released, so ESLint 10 is effectively unusable with any TypeScript + Next.js stack today.

Rather than stay locked at ESLint 9 indefinitely waiting for the ecosystem, the lint+format layer was migrated to **Biome 2**:

- One Rust binary instead of three Node tools (ESLint, Prettier, Stylelint).
- Drops the `typescript-eslint` dependency chain entirely — no more "waiting for X.0 to support Y.0."
- Handles JS, TS, JSX, TSX, JSON, and CSS in one config (`biome.json`).
- 10–100× faster than the ESLint+Prettier combination.
- Modern recommended rule set with surgical per-file overrides where the project's deliberate patterns trigger noise (literal nav anchor IDs, `!` non-null assertions in tests, `${var}` literal text in the how-its-built page's code samples).
- The pre-commit lint-staged hook now runs `biome check --write` over staged files.

What was lost: a few Next.js-specific lint rules from `eslint-config-next` (e.g. `@next/next/no-html-link-for-pages`). These are anti-pattern warnings rather than correctness bugs; the value loss is small.
...
| `biome.json` | Lint/format config, not application code |
...
- **Switching frameworks to Astro/Svelte/Solid** — considered briefly; Next.js 16 + React 19 remains the choice because the user wants future flashy/animation extensibility, and React still has the strongest animation ecosystem (Motion, GSAP, R3F, Lottie).
